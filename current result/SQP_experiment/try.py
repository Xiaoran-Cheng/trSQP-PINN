...
class SQP_Optim:
    ...
    # def get_recovered_dict(self, params, flatted_target):
    #         shapes, sizes = self.get_shapes_size(params)
    ...
    #         flatted_target_df.columns = params["params"].keys()
    ...
    def SQP_optim(self, params, num_iter, maxiter, condition, decrease_factor, init_stepsize):
        ...
        for _ in tqdm(range(num_iter)):
            ...
            subarrays = np.split(flatted_delta_params, np.cumsum(sizes)[:-1])
            reshaped_arrays = [subarray.reshape(shape) for subarray, shape in zip(subarrays, shapes)]
            flatted_delta_params_df = pd.DataFrame(np.array(reshaped_arrays, dtype=object).\
                        reshape(2,len(self.feature))).applymap(lambda x: x)
            flatted_delta_params_df.columns = params["params"].keys()
            flatted_delta_params_df.index = ["bias", "kernel"]
            delta_params = FrozenDict({"params": flatted_delta_params_df.to_dict()})
            ...
            print(self.optim_components.obj(params))
    ...
    for activation_input in ['tanh']:
        ...
        for beta in beta_list:
            ...
            if experiment == "SQP_experiment":
                optim_components = OptimComponents(model, data, sample_data, IC_sample_data, ui, beta)
                sqp_optim = SQP_Optim(model, optim_components, qp, features, group_labels, hessiam_param, M)
                ...
                absolute_error, l2_relative_error, eval_u_theta = \
                    sqp_optim.evaluation(updated_params, N, eval_data, eval_ui)
                
            else:
                if experiment == "PINN_experiment":
                    loss = PINN(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "l1_Penalty_experiment":
                    loss = l1Penalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "l2_Penalty_experiment":
                    loss = l2Penalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "linfinity_Penalty_experiment":
                    loss = linfinityPenalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "Han_penalty_experiment":
                    loss = HanPenalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "Cubic_Penalty_experiment":
                    loss = CubicPenalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "Augmented_Lag_experiment":
                    loss = AugLag(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "Pillo_Penalty_experiment":
                    loss = PilloPenalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "New_Augmented_Lag_experiment":
                    loss = NewAugLag(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
                elif experiment == "Fletcher_Penalty_experiment":
                    loss = FletcherPenalty(model, data, sample_data, IC_sample_data, ui, beta, \
                                N, M)
