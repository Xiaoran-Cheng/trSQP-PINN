...
title = ["ALM", "Penalty method", "trSQP-PINN"]
...
for method in file_path:
    ...
    if os.path.basename(os.path.dirname(method)) == "with_pretrain_experiment_result" and os.path.basename(os.path.dirname(os.path.dirname(method))) == "Transport":
        visual.heatmap(eval_data, eval_u_theta, "",os.path.basename(os.path.dirname(method)), experiment=dicts[os.path.basename(method)], nt=nt, xgrid=xgrid, color_bar_bounds=color_bar_bounds, title=title[idx])
        idx += 1
    else:
...
if system == "convection":
    visual.heatmap(eval_data, eval_ui[0], "","True_sol_heatmap", experiment="True_sol", nt=nt, xgrid=xgrid, color_bar_bounds=color_bar_bounds, title="Exact Solution")
else:
