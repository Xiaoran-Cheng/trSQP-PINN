...
if system == "reaction_diffusion":
    for i in get_params_dirs(system):
        method = "penalty" if "l2^2" in os.path.basename(i) else ("ALM" if "Augmented_Lag" in os.path.basename(i) else ("SQP" if "SQP" in os.path.basename(i) else "ERROR"))
    ...
    for i in get_params_dirs(system):
        if os.path.basename(os.path.dirname(i)) == "Varying_System_Complexity_Coefficient":
            method = "penalty" if "l2^2" in os.path.basename(i) else ("ALM" if "Augmented_Lag" in os.path.basename(i) else ("SQP" if "SQP" in os.path.basename(i) else "ERROR"))
            experiment_type = os.path.basename(os.path.dirname(i))
