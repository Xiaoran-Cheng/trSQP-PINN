...
qp = CvxpyQP(solver='OSQP', implicit_diff_solve=True) # "OSQP", "ECOS", "SCS"
SQP_num_iter = 5
...
line_search_decrease_factor = 0.9
...
error_df_list = []
for merit_func_penalty_param in [1, 10, 10**2, 10**3, 10**4, 10**5, 10**6]:
    for experiment in ['SQP_experiment']:
        ...
        for activation_input in ['sin']:
            ...
            for beta in beta_list:
                ...
                if experiment != "SQP_experiment":
                    visual.line_graph(total_loss_list, "Total_Loss", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param)
                visual.line_graph(total_eq_cons_loss_list, "Total_eq_cons_Loss", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param)
                visual.line_graph(total_l_k_loss_list, "Total_l_k_Loss", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param)
                if experiment == "SQP_experiment":
                    visual.line_graph(kkt_residual_list, "KKT_residual", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param)


                visual.line_graph(eval_ui[0], "True_sol_line", experiment="", activation="", beta=merit_func_penalty_param)
                visual.line_graph(eval_u_theta, "u_theta_line", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param)
                visual.heatmap(eval_data, eval_ui[0], "True_sol_heatmap", experiment="", beta=merit_func_penalty_param, activation="", nt=nt, xgrid=xgrid)
                visual.heatmap(eval_data, eval_u_theta, "u_theta_heatmap", experiment=experiment, activation=activation_name, beta=merit_func_penalty_param, nt=nt, xgrid=xgrid)
