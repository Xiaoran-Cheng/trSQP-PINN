...
class PINN:
    ...
    def IC_cons(self, params):
        ...
        return jnp.square(jnp.linalg.norm(Transport_eq(beta=self.beta).solution(\
        self.IC_sample_data[:,0], self.IC_sample_data[:,1]) - u_theta, ord=2))
    
    
    def pde_cons(self, params):
        grad_x = jacfwd(self.model.u_theta, 1)(params, self.sample_data)
        return jnp.square(jnp.linalg.norm(Transport_eq(beta=self.beta).pde(jnp.diag(grad_x[:,:,0]),\
                                         jnp.diag(grad_x[:,:,1])), ord=2))
